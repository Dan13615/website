...
export default function CartePage() {
	...
	const breakfast_formulas = [
		{
			name: "The Vital",
			price: 10.9,
			description: ["Hot drink (tea, cappucino or chocolate)", "Special and rustic breads", "Homemade Jam"],
		},
		{
			name: "The Greedy",
			price: 16.9,
			description: ["Hot drink (tea, cappucino or chocolate)", "Special and rustic breads", "Homemade Jam", "Eggs-bacon (fried, omelette or scrambled)", "Fresh fruits salad"],
		},
		{
			name: "The Morning plate",
			price: 13.9,
			description: ["Omelette or scrambled eggs", "(Nature or with chees) + Bacon", "Ham and cheese"],
		},
	];
	...
	const brunch = {
		name: "Brunch",
		price: 27,
		description: ["Hot drink (tea, cappucino or chocolate)", "Special and rustic breads", "Scrambled Eggs, bacun, mesclun", "Charcuterie", "Smoked salmon", "Cheese", "Homemade Jam", "Fresh fruits salad", "Freshly squeezed orange juice", "Whipped cream cheese", "Gluten-free bread supplement: 1.50€"],
	};
				...
				<div className="flex flex-row w-full gap-24">
					...
					<div className="flex flex-col w-1/2">
						<div className="flex flex-col w-full justify-center items-center gap-12">
							...
							<div className="flex flex-col w-full justify-center items-center gap-4">{brunch.description.map((e, i) => [<div className="textStyle-text  text-center">{e}</div>, i < brunch.description.length - 1 && <div className="w-8 border-solid border-2 rounded-md"></div>])}</div>
						</div>
					</div>
				</div>
			</PageHeaderImage>
			<PageHeaderImage bg_color="brown" title="Les Tartines Poilanes" subtitle="Pain cuit au feu de bois, garni et gratiné, accompagné de mesclun Niçois." image="/img/tartine.jpeg">
				{poilane.map((p, i) => (
					<Product name={p.name} price={p.price.toFixed(2)} description={p.description} />
				))}
			</PageHeaderImage>
		</div>
	);
