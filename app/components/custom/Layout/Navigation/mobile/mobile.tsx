...
export function NavbarMobile() {
  ...
  return (
    <>
      <div className="fixed top-0 right-0 z-50 p-4 w-full flex color-bg-green items-center justify-between border-b-2 color-border-gold">
        <div>
        ...
        <div className={`${open ? css.slideIn : css.slideOut} ${css.sidebar}`}>
          <div className={css.navbarLinksContainer}>
            {pagesConfig.map((page) => {
              ...
              return (
                <div key={page.name}>
                  <a href={page.path} className={`text-2xl font-extrabold transition-colors duration-500 ${isActive ? "color-text-gold" : "color-text-white group-hover:text-yellow-300"}`}>
                    {page.name}
                  </a>
                </div>
              );
